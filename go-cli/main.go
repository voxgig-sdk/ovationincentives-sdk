// Command Ovationincentives-cli is a CLI + REPL for the Ovationincentives SDK,
...
const prompt = "ovationincentives"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "code"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      OVATIONINCENTIVES_APIKEY (api key), OVATIONINCENTIVES_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: OVATIONINCENTIVES_APIKEY carries the API key and
	// OVATIONINCENTIVES_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("OVATIONINCENTIVES_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("OVATIONINCENTIVES_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewOvationincentivesSDK(opts)
