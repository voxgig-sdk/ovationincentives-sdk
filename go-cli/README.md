# ovationincentives-cli

boru-driven command-line client **and** interactive REPL for the Ovationincentives
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ovationincentives-cli)
make build

# 2. See usage (words, entities, env vars)
./ovationincentives-cli --help

# 3. Provide credentials once, via the environment
export OVATIONINCENTIVES_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./ovationincentives-cli load 1 code            # {id:1} shorthand
./ovationincentives-cli load '{id:1}' code       # explicit match map

# 5. Override the API base URL for a single call
OVATIONINCENTIVES_BASE=https://api.example.com ./ovationincentives-cli load 1 code

# 6. No arguments -> interactive REPL
./ovationincentives-cli
ovationincentives> load 1 code
ovationincentives> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ovationincentives-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/ovationincentives-cli load 1 code
   ```
...
```sh
./ovationincentives-cli load 1 code          # scalar shorthand for {id:1}
./ovationincentives-cli load '{id:1}' code     # explicit match map
```
...
./ovationincentives-cli load 1 code
```
...
Run with no arguments to open a REPL (prompt `ovationincentives>`). Each line is
evaluated as its own boru expression:

```text
$ ./ovationincentives-cli
ovationincentives> load 1 code
ovationincentives> /help
ovationincentives> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `code`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/ovationincentives-cli`. |
...
(`./ovationincentives-cli <expr>`) and interactively (the REPL), and expressions compose the
