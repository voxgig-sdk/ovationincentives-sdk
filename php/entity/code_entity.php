...
// Ovationincentives SDK Code entity
...
class CodeEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new CodeEntity($this->_client, $opts);
    }

    /**
     * @param Code|array $args Code data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = OvationincentivesHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Code|array The current Code data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Code fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = OvationincentivesHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Code fields).
     */
     ...
     * Load a single Code.
     *
     * @param CodeLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed CodeLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Code|array The loaded Code as an assoc-array at the
     *   SDK boundary; throws OvationincentivesError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = OvationincentivesHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     * Create a new Code.
     *
     * @param CodeCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed CodeCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Code|array The created Code as an assoc-array at the
     *   SDK boundary; throws OvationincentivesError on failure (item-5 convention).
     */
