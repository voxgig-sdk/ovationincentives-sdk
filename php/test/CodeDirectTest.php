...
class CodeDirectTest extends TestCase
{
    public function test_direct_load_code(): void
    {
        $setup = code_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-code", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
...
function code_direct_setup($mockres)
{
    ...
    if ($live) {
        ...
        $client = new OvationincentivesSDK($merged_opts);
        return [
            "client" => $client,
            "calls" => $calls,
            "live" => true,
