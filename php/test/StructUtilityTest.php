...
// Ovationincentives SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = OvationincentivesOmni::makeStructRunner(
    ...
    {
        return OvationincentivesOmni::objmodel(OvationincentivesOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return OvationincentivesOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return OvationincentivesOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(OvationincentivesOmni::structfix($now, Struct::undef()))
                        !== json_encode(OvationincentivesOmni::structfix($val, Struct::undef()))) {
