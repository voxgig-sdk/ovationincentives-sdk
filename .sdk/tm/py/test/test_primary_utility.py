...
from projectname_sdk import OvationincentivesSDK
from projectname_sdk.core.spec import OvationincentivesSpec
from projectname_sdk.core.result import OvationincentivesResult
from projectname_sdk.core.response import OvationincentivesResponse
from projectname_sdk.core.operation import OvationincentivesOperation
from projectname_sdk.core.error import OvationincentivesError
from projectname_sdk.core import helpers
from projectname_sdk.feature.base_feature import OvationincentivesBaseFeature
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return OvationincentivesError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_exists(self):
        client = OvationincentivesSDK.test(None, None)
    ...
    def test_done_basic(self):
        ...
        def subject(entry):
            ...
            except OvationincentivesError as e:
                return None, e
    ...
    def test_make_error_no_throw(self):
        ...
        ctx.result = OvationincentivesResult({
            "ok": False,
            "resdata": {"id": "safe01"},
        })
    ...
    def test_feature_init_basic(self):
        init_client = OvationincentivesSDK.test(None, None)
        ...
        class TestInitFeature(OvationincentivesBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(OvationincentivesBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_fetcher_live(self):
        ...
        live_client = OvationincentivesSDK({
    ...
    def test_fetcher_blocked_test_mode(self):
        ...
        blocked_client = OvationincentivesSDK({
            "base": "http://localhost:8080",
            "system": {
                "fetch": mock_fetch,
            },
    ...
    def test_make_fetch_def_basic(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = OvationincentivesSpec({
            "base": "http://localhost:8080",
            "prefix": "/api",
            "path": "items/{id}",
            "suffix": "",
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = OvationincentivesSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = OvationincentivesSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = OvationincentivesOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = OvationincentivesSDK.test(None, setup_opts)
