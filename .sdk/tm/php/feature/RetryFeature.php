...
// Ovationincentives SDK retry feature
...
class OvationincentivesRetryFeature extends OvationincentivesBaseFeature
{
    ...
    public function init(OvationincentivesContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (OvationincentivesContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(OvationincentivesContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(OvationincentivesContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
