...
class OvationincentivesMakeRequest
{
    public static function call(OvationincentivesContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new OvationincentivesResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new OvationincentivesResponse($fetched);
        } else {
