...
class OvationincentivesMakeError
{
    public static function call(?OvationincentivesContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new OvationincentivesContext([], null);
        }
        $op = $ctx->op ?? new OvationincentivesOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new OvationincentivesResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof OvationincentivesError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new OvationincentivesError('', $msg, $ctx);
        ...
        if ($err instanceof OvationincentivesError) {
