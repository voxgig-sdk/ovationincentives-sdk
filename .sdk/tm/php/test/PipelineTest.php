...
// Ovationincentives SDK pipeline test
//
...
{
    private static function utility(): OvationincentivesUtility
    {
        return new OvationincentivesUtility();
    }
    ...
    private static function ctx(array $over = []): OvationincentivesContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new OvationincentivesContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new OvationincentivesOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof OvationincentivesError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new OvationincentivesOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = OvationincentivesMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new OvationincentivesOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = OvationincentivesMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new OvationincentivesOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = OvationincentivesMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OvationincentivesSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = OvationincentivesMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new OvationincentivesResponse([]),
            'result' => new OvationincentivesResult([]),
        ]);
        [, $err] = OvationincentivesMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new OvationincentivesSpec([]),
            'response' => null,
            'result' => new OvationincentivesResult([]),
        ]);
        [, $err] = OvationincentivesMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new OvationincentivesSpec(['step' => 's']),
            'response' => new OvationincentivesResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new OvationincentivesResult([]),
        ]);
        [, $err] = OvationincentivesMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new OvationincentivesSpec(['step' => 's']),
            'response' => new OvationincentivesResponse(self::resp(200, ['v' => 1])),
            'result' => new OvationincentivesResult([]),
        ]);
        [, $err] = OvationincentivesMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new OvationincentivesSpec(['step' => 's']),
            'response' => new OvationincentivesResponse(self::resp(200, ['v' => 2])),
            'result' => new OvationincentivesResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        OvationincentivesMakeResponse::call($ctx);
    ...
    {
        $preset = new OvationincentivesResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new OvationincentivesSpec([]),
            'response' => new OvationincentivesResponse([]),
            'result' => new OvationincentivesResult([]),
        ...
        [$got, $err] = OvationincentivesMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new OvationincentivesResult([])]);
        [, $err] = OvationincentivesMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new OvationincentivesSpec([]), 'result' => null]);
        [, $err] = OvationincentivesMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OvationincentivesSpec(['step' => 's']),
            'result' => new OvationincentivesResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new OvationincentivesOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OvationincentivesMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OvationincentivesSpec(['step' => 's']),
            'result' => new OvationincentivesResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new OvationincentivesOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OvationincentivesMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OvationincentivesResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new OvationincentivesSpec([]),
            'result' => new OvationincentivesResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = OvationincentivesMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = OvationincentivesMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new OvationincentivesError('boom', 'boom');
        $utility->fetcher = function (OvationincentivesContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OvationincentivesSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = OvationincentivesMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (OvationincentivesContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = OvationincentivesMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(OvationincentivesResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        OvationincentivesMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (OvationincentivesContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OvationincentivesSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = OvationincentivesMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new OvationincentivesResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new OvationincentivesSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = OvationincentivesMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = OvationincentivesMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = OvationincentivesMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new OvationincentivesResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, OvationincentivesDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new OvationincentivesResult(['ok' => false])]);
        $this->expectException(OvationincentivesError::class);
        OvationincentivesDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new OvationincentivesResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', OvationincentivesMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        OvationincentivesMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new OvationincentivesBaseFeature();
        $b = new OvationincentivesBaseFeature();
        OvationincentivesFeatureAdd::call($ctx, $a);
        OvationincentivesFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): OvationincentivesBaseFeature
    {
        $f = new OvationincentivesBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        OvationincentivesFeatureAdd::call($ctx, self::named_feature('a'));
        OvationincentivesFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        OvationincentivesFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        OvationincentivesFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        OvationincentivesFeatureAdd::call($ctx, $replace);
        ...
        OvationincentivesFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): OvationincentivesContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new OvationincentivesSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = OvationincentivesPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        OvationincentivesPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new OvationincentivesResponse(['headers' => null]),
            'result' => new OvationincentivesResult([]),
        ]);
        OvationincentivesResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        OvationincentivesResultBody::call($ctx);
