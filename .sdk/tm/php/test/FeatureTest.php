...
// Ovationincentives SDK feature test
//
...
class FtCtrl extends OvationincentivesControl
{
...
class FtClient
{
    ...
    private OvationincentivesUtility $utility;

    public function __construct(OvationincentivesUtility $utility, array $options)
    {
    ...
    public function get_utility(): OvationincentivesUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (OvationincentivesContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new OvationincentivesUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (OvationincentivesContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = OvationincentivesConfig::make_feature($name);
    ...
    private static function buildUrl(OvationincentivesSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof OvationincentivesError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new OvationincentivesResponse($fetched);
        }
    ...
    private function fail(OvationincentivesContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (OvationincentivesContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
