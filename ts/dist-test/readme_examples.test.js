...
const __1 = require("..");
const SDK_NAME = 'OvationincentivesSDK';
...
const TEST_SEED = { "entity": { "code": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+OvationincentivesSDK\b/.test(code) ||
        /\bOvationincentivesSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+OvationincentivesSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/OvationincentivesSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
