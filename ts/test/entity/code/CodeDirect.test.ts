...
describe('CodeDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new OvationincentivesSDK({
  ...
  test('direct-load-code', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-code', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['OVATIONINCENTIVES_TEST_CODE_ENTID']
