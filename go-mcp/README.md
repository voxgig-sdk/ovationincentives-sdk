# ovationincentives-mcp

[MCP](https://modelcontextprotocol.io) server exposing the Ovationincentives SDK as
two agent tools — `ovationincentives_list` and `ovationincentives_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ovationincentives-mcp)
make build

# 2. Provide credentials via the environment
export OVATIONINCENTIVES_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user ovationincentives \
  -- /absolute/path/to/ovationincentives-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./ovationincentives-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// ovationincentives_list: first page of records
{ "entity": "code" }
{ "entity": "code", "query": { } }

// ovationincentives_load: one record by id
{ "entity": "code", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ovationincentives-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user ovationincentives \
     -- "$PWD"/dist/*/ovationincentives-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `ovationincentives_list` and `ovationincentives_load` tools now appear
   in new sessions. Ask the agent to *"list code using ovationincentives"*
   and it calls `ovationincentives_list` with `{"entity":"code"}`.
...
```sh
export OVATIONINCENTIVES_APIKEY=sk_live_xxx            # API key
export OVATIONINCENTIVES_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `ovationincentives_list` tool
...
### Call the `ovationincentives_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `ovationincentives_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `ovationincentives_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `OVATIONINCENTIVES_APIKEY` | API key sent with every request. |
| `OVATIONINCENTIVES_BASE` | Optional override of the API base URL. |
...
```sh
./ovationincentives-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ovationincentives_load","arguments":{"entity":"code","query":{"id":1}}}}'
```
