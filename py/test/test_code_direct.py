...
class TestCodeDirect:

    def test_should_direct_load_code(self):
        setup = _code_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-code", "live" if setup["live"] else "unit")
        if _skip:
...
def _code_direct_setup(mockres):
    ...
    live = env.get("OVATIONINCENTIVES_TEST_LIVE") == "TRUE"
    ...
    if live:
        ...
        client = OvationincentivesSDK(merged_opts)
        return {
            "client": client,
            "calls": calls,
            "live": True,
