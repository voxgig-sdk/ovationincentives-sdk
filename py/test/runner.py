# Ovationincentives SDK test runner
...
class OvationincentivesTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            OvationincentivesTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = OvationincentivesTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = OvationincentivesTestRunner.getenv("OVATIONINCENTIVES_TEST_LIVE")
        override = OvationincentivesTestRunner.getenv("OVATIONINCENTIVES_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = OvationincentivesTestRunner.getenv(key)
        ...
        explain = OvationincentivesTestRunner.getenv("OVATIONINCENTIVES_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["OVATIONINCENTIVES_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if OvationincentivesTestRunner._test_control is not None:
            return OvationincentivesTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                OvationincentivesTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            OvationincentivesTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = OvationincentivesTestRunner.load_test_control()
...
def load_env_local():
    OvationincentivesTestRunner.load_env_local()


def env_override(m):
    return OvationincentivesTestRunner.env_override(m)


def entity_data(v):
    return OvationincentivesTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return OvationincentivesTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return OvationincentivesTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return OvationincentivesTestRunner.load_test_control()


def live_client_options():
    return OvationincentivesTestRunner.live_client_options()


def live_delay_ms():
    return OvationincentivesTestRunner.live_delay_ms()
