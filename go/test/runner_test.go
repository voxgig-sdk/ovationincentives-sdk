...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("OVATIONINCENTIVES_TEST_LIVE") == "TRUE" ||
		os.Getenv("OVATIONINCENTIVES_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("OVATIONINCENTIVES_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.OvationincentivesSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.OvationincentivesSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.OvationincentivesError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.OvationincentivesSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.OvationincentivesError{Msg: msg, Code: code}
}
