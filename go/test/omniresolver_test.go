...
import (
	...
	omni "github.com/voxgig-sdk/ovationincentives-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.OvationincentivesError); is {
				out := map[string]any{
					"name":    "OvationincentivesError",
...
func omniCtx(arg any, client *sdk.OvationincentivesSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.OvationincentivesSDK); is {
			client = live
