# Ovationincentives Lua SDK Reference

Complete API reference for the Ovationincentives Lua SDK.
...
```lua
local result, err = client:Code():create({
})
```
...
```lua
local result, err = client:Code():load({ id = "code_id" })
```
