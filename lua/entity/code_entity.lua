...
local CodeEntity = {}
CodeEntity.__index = CodeEntity


function CodeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CodeEntity)
  self._name = "code"
  self._client = client
...
function CodeEntity:get_name()
  return self._name
end


function CodeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CodeEntity.new(self._client, opts)
end
...
function CodeEntity:mark_deleted()
  self._deleted = true
end


function CodeEntity:deleted()
  return true == self._deleted
end


function CodeEntity:data_set(args)
  if args ~= nil then
...
function CodeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CodeEntity:match_set(args)
  if args ~= nil then
...
function CodeEntity:stream(action, args, callopts)
...
---@param reqmatch CodeLoadMatch
---@param ctrl? table
---@return Code
---@return string? err
function CodeEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata CodeCreateData
---@param ctrl? table
---@return Code
---@return string? err
function CodeEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function CodeEntity:_run_op(ctx, post_done)
  local utility = self._utility
