...
require("ovationincentives_types")
...
local OvationincentivesSDK = {}
OvationincentivesSDK.__index = OvationincentivesSDK
...
OvationincentivesSDK._make_feature = _make_feature


function OvationincentivesSDK.new(options)
  local self = setmetatable({}, OvationincentivesSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function OvationincentivesSDK:options_map()
...
function OvationincentivesSDK:get_utility()
  return Utility.copy(self._utility)
end


function OvationincentivesSDK:get_root_ctx()
  return self._rootctx
end


function OvationincentivesSDK:prepare(fetchargs)
...
function OvationincentivesSDK:direct(fetchargs)
...
function OvationincentivesSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function OvationincentivesSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "OvationincentivesSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function OvationincentivesSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function OvationincentivesSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "OvationincentivesSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function OvationincentivesSDK:Code(data)
...
function OvationincentivesSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = OvationincentivesSDK.new(sdkopts)
  sdk.mode = "test"
